...
Description:
    ...
    directory structure under DATA/FINAL/full_data, preserving the original
    labels and ensuring each species remains proportionally represented.

Notes:
    - Input data must follow this structure:
         DATA/INITIAL/full_data/<train|test|val>/<img|ann>/
...
INPUT_BASE_DIR = Path(__file__).parent.parent / "DATA" / "INITIAL" / "full_data"

# Base directory where final stratified split will be written
OUTPUT_BASE_DIR = Path(__file__).parent.parent / "DATA" / "FINAL" / "full_data"
