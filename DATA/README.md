...
- Initial Data: There are two different sets with the same layout, one for the sample data and one for the full data (lableled accordingly). The dataset was stored in a zip folder with a separate test, train, and val folder that was split by the original project authors. Within the separate folders, it contains the img folder with the actual images and an ann folder containing the classifiers for each tree species.
- Final Data: Again there will be two different final datasets, one for the sample data and one for the full data. The dataset will be stored in a single directory with a 70-20-10 proportional split, named according to what each is used for (train, validate, tets).
